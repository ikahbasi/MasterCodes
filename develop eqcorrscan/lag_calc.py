...
def xcorr_pick_family(family, stream, shift_len=0.2, min_cc=0.4,
    ...
    for i, detection_id in enumerate(detection_ids):
        ...
        correlations = ccc[i]
        os.makedirs(plotdir, exist_ok=True)                                          # iman kahbasi
        np.save(file=f'{plotdir}/{detection_id}', arr=correlations)                  # iman kahbasi
        ...
        for correlation, stachan in zip(correlations, picked_chans):
            ...
            if cc_max > min_cc:
                ...
                print('cc_max < min_cc')
                num_of_peaks = len(
                    scipy.signal.find_peaks(correlation, height=cc_max/1.5)[0]
                    )
