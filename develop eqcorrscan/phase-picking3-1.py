...
#        arr = arrs[ii]
        ...
        plt.axhspan(-1*rms_threshold, rms_threshold, facecolor='#2ca02c',
                    alpha=0.5, label='2*rms zone')
        ...
        plt.hlines(
            [Strict_threshold, Strict_threshold*(-1)], 0, len(arr),
            'b', linestyles='dashdot', label='Strict threshold')
        # find my maximume peaks
        if Max > Strict_threshold:
            detection_max = [Max_index, Max]
        else:
            plt.hlines(0.75*Max, 0, len(arr), 'g', label='is it a unique pick?')
            num_of_peaks = len(find_peaks(arr, height=0.75*Max)[0])
            if  num_of_peaks == 1:
                detection_max = [Max_index, Max]
            elif num_of_peaks > 1:
                detection_max = [None, None]
        # find my minimume peaks
        if Min < (-1)*Strict_threshold:
            detection_min = [Min_index, Min]
        else:
            plt.hlines(0.75*Min, 0, len(arr), 'g', label='is it a unique pick?')
            num_of_peaks = len(find_peaks(-1*arr, height=-0.75*Min)[0])
            if num_of_peaks==1:
                detection_min = [Min_index, Min]
            else:
        ...
        if rms_threshold is not None:
            rms_threshold = round(rms_threshold, 2)
        text = 'mean is: %.2f\nmax is: %.2f\nmin is: %.2f' % (rms_threshold, Max, Min)
