...
indx = list_days.index('../days/2014-10-21')
list_days = list_days[indx:]

list_stations = ['SKAR', 'EINS', 'ABTF', 'MIME', 'DLRN', 'TIAK', 'JHAD', 'GLGL', 'BARE'] #'ANJR', 'MAHO', 'HONG', 'TONL',

for station in list_stations:
    print(station)
    st = Stream()
    list_files = glob.glob(join('..', 'days', '*', f'{station}.SZ*'))
    for f in list_files:
        tmp_st = read(f)
        for tr in tmp_st:
            st += tr
    ...
    try:
        ...
        #print(st)
        #if st[0].stats.station not in ['MAHO', 'EINS']:
        #    continue
        st.detrend('constant')
        st.merge(method=1)
        tr = st[0]
        print(tr)
        ppsd = PPSD(tr.stats, paz1, ppsd_length=360.0, overlap=0.2)
        ppsd.add(st)
        outpath = join('PSD', tr.stats.station, tr.stats.channel)
        os.makedirs(outpath, exist_ok=True)
        name = f'{tr.id}_PPSD..npz'
        ppsd.save_npz(filename=join(outpath, name))
        name = f'{tr.id}_PPSD.png'
